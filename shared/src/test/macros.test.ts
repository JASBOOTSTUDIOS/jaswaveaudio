/**
 * Tests del registro de macros.
 */

import { describe, it, expect, vi } from 'vitest';
import { crearRegistroMacros } from '../state/macros';
...
import type { LoopMode, PunchMode } from '../types/transport';
...
describe('Macros', () => {
  it('registra, lista y cancela una macro', async () => {
    const registro = crearRegistroMacros();
    registro.register({ id: 'm1', name: 'm1', commands: [], parameters: {} });
    expect(registro.list().map(m => m.id)).toContain('m1');
    registro.unregister('m1');
    expect(registro.get('m1')).toBeUndefined();
  });

  it('ejecuta macro simple y reporta progreso', async () => {
    const registro = crearRegistroMacros();
    registro.register({ id: 'm1', name: 'm1', commands: [], parameters: {} });
    ...
    const executor = {
      batch: async () => ({ success: true, results: [], rolledBack: false }),
      obtenerEstado: () => estadoBase(),
    } as any;

    const resultado = await registro.execute('m1', executor, { onProgress });
    expect(resultado.success).toBe(true);
    expect(onProgress).toHaveBeenCalledTimes(1);
    expect(onProgress.mock.calls[0][0].macroId).toBe('m1');
    expect(onProgress.mock.calls[0][0].estado).toBe('completada');
  });

  it('falla cuando la macro no existe', async () => {
    const registro = crearRegistroMacros();
    const resultado = await registro.execute('missing', {} as any);
    expect(resultado.success).toBe(false);
  });

  it('grupos de macros funcionan', () => {
    const registro = crearRegistroMacros();
    registro.createGroup({ id: 'g1', nombre: 'g1', descripcion: 'd1', macros: ['m1'], secuencial: true });
    expect(registro.getGroup('g1')).toBeDefined();
  });
});
