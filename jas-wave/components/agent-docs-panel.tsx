...
import { FilePlus, FileText, PlayCircle, Trash2 } from 'lucide-react'
    ...
    <div className="flex h-full min-h-0 flex-col bg-panel">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          {evalMsg ? (
            <div className="border-b border-border/60 px-2 py-1 text-[10px] text-muted-foreground">{evalMsg}</div>
          ) : (
            <div className="border-b border-border/60 px-2 py-1 text-[10px] text-muted-foreground">
              Editable por ti y por la IA. La sección «Notas del usuario» se conserva al reevaluar.
            </div>
          )}
          <textarea
            value={draft}
